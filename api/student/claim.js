...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!student.portal_enabled && student.status !== 'active') return res.status(403).json({ error: 'Student portal access is disabled' });
    ...
    const { data: updated, error: e3 } = await supabase.from('student_profiles').update(patch).eq('id', student.id).select('id,admission_no,roll_no,class_level,stream,academic_session,full_name,phone,email,registration_no,portal_enabled').single();
    if (e3) throw e3;
